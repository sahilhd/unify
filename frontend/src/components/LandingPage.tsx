...
export default function LandingPage() {
  ...
  return (
    <div className="relative min-h-screen flex flex-col overflow-x-hidden bg-gradient-to-br from-gray-950 via-gray-900 to-gray-800">
      ...
      <main className="flex-1 flex flex-col items-center justify-center text-center px-4 z-10 relative">
        <section className="pt-24 pb-16">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-16 justify-center">
            <button className="px-10 py-4 bg-gradient-to-r from-purple-500 to-blue-600 hover:from-purple-600 hover:to-blue-700 rounded-full text-lg font-semibold text-white shadow-xl transition-all duration-200 hover:shadow-glow" onClick={() => navigate('/login')}>Get Started</button>
            <button className="px-10 py-4 bg-white/10 hover:bg-white/20 rounded-full text-lg font-semibold text-white shadow-xl border border-white/10 transition-all duration-200 hover:shadow-soft" onClick={() => window.open('https://github.com/sahilhd/unify', '_blank')}>View on GitHub</button>
          </div>
        </section>
        <section className="w-full max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 mb-24">
          {features.map((feature) => (
            <div key={feature.title} className="backdrop-blur-lg bg-white/5 border border-white/10 rounded-2xl p-8 shadow-2xl flex items-center gap-6 transition-all duration-300 hover:scale-[1.02] hover:shadow-glow group">
              <span className="text-5xl md:text-6xl drop-shadow-lg group-hover:scale-110 transition-transform duration-300">{feature.icon}</span>
              <div className="text-left">
                <h3 className="text-2xl font-bold text-purple-200 mb-3 tracking-tight group-hover:text-purple-100 transition-colors">{feature.title}</h3>
        ...
        <section className="w-full max-w-4xl mx-auto mb-20">
          <div className="backdrop-blur-lg bg-gradient-to-r from-purple-900/30 via-blue-900/20 to-fuchsia-900/20 border border-white/10 rounded-2xl p-12 shadow-2xl">
            <h2 className="text-4xl font-bold text-purple-300 mb-6 tracking-tight">Why UniLLM?</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
              <ul className="list-disc list-inside text-gray-200 text-lg font-light space-y-3">
      ...
      <footer className="text-center text-gray-500 py-12 text-base font-light border-t border-white/10 bg-gradient-to-t from-gray-900/80 to-transparent z-10 relative">
