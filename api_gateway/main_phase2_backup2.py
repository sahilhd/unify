...
# Chat completion endpoint (enhanced with streaming support)
...
async def chat_completion(
    ...
    """Enhanced chat completion with billing and streaming support"""
    import sys
    print(f"[DEBUG] Executing chat_completion from file: {__file__}")
    print(f"[DEBUG] Current line number: {sys._getframe().f_lineno}")
    ...
    try:
        if request.stream:
            response_generator = llm_client.chat(
                model=request.model,
            ...
            return StreamingResponse(
                stream_chat_response(response_generator, current_user, request, db, start_time),
            ...
            # For non-streaming, call the LLM and handle response normally
            print("[DEBUG] About to call llm_client.chat() with stream=False")
            ...
            print('[UniLLM DEBUG] Non-streaming mode: continuing to normal response handling')
        ...
        # Log error - don't try to access .provider on generator
        usage_log = UsageLog(
            user_id=current_user.id,
            model=request.model,
            provider="unknown",
...
async def stream_chat_response(
    ...
    try:
        ...
        # After streaming is complete, calculate final metrics
        response_time = int((time.time() - start_time) * 1000)
        
        # Estimate tokens (rough calculation)
        total_chars = sum(len(msg.get("content", "")) for msg in request.messages) + len(full_content)
        estimated_tokens = max(total_chars // 4, 1)
        
        # Calculate cost
        provider = chunk.provider if chunk_count > 0 else "unknown"
        cost = calculate_cost(provider, request.model, estimated_tokens)
        ...
        tb = traceback.format_exc()
        print(f"[UniLLM] Exception in streaming: {e}\n{tb}", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    # Extract only the numeric part of PORT in case it contains extra data
    try:
        port = int(PORT_RAW.split()[0])  # Take only the first part before any space
